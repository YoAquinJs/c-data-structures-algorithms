...
const char args_doc[] =
    "must always select one of --all, --category and --test";
...
static struct argp_option options[] = {
    ...
    {"print-on-fail", 'p', NULL, 0,
     "Optional flag to print test output on failure", 2},
    {0}};
    ...
    switch (key) {
        case 'a':  // --all
            if (args->category || args->test) {
                argp_error(
                    state,
                    "Option --all cannot be used with --category or --test");
            }
            args->all = 1;
            break;
        case 'c':  // --category
            if (args->all || args->test) {
                argp_error(
                    state,
                    "Option --category cannot be used with --all or --test");
            }
            args->category = arg;
            break;
        case 't':  // --test
            if (args->all || args->category) {
                argp_error(
                    state,
                    "Option --test cannot be used with --all or --category");
            }
            args->test = arg;
            break;
        ...
        case ARGP_KEY_END:
            if (!args->all && !args->category && !args->test) {
                argp_error(
                    state,
                    "one of --all, --category, or --test must be specified");
            }
            break;
        default:
